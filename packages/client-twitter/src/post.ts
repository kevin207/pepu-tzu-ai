...
// import { saveBase64Image, saveHeuristImage } from "./index";
// import saveBase64Image from "./index";
// import saveHeuristImage from "./index";
// import { saveBase64Image } from "../../plugin-image-generation/src/index";
...
export class TwitterPostClient {
    ...
    // Default value of postImmediately is false
    async start(postImmediately: boolean = true) {
        ...
        if (postImmediately) {
            elizaLogger.log(
                "Posting tweet immediately after start up is enabled in the env file"
            );
            this.generateNewTweet();
        } else {
            elizaLogger.log(
                "Posting tweet immediately after start up is disabled in the env file"
            );
        }
    ...
    private async generateNewTweet() {
        elizaLogger.log("Generating new tweet to post");
        ...
        try {
            ...
            elizaLogger.log("Current State for tweet posting: ", state);
            ...
            //         await this.client.twitterClient.uploadMedia(filepath);
            //     mediaId = mediaUploadResponse.media_id_string;
            // } else {
            //     elizaLogger.warn("Failed to generate image for tweet");
            // }
            ...
            try {
                ...
                elizaLogger.error("Error sending tweet post:", error);
            }
        } catch (error) {
            elizaLogger.error("Error generating new tweet post:", error);
        }
    }
}
