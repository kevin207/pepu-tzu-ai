...
Remember to include an action if the current post text includes a prompt that asks for one of the available actions mentioned above (does not need to be exact)
` + messageCompletionFooter;
...
export class TwitterInteractionClient {
    ...
    async start() {
        const handleTwitterInteractionsLoop = () => {
            ...
            setTimeout(
                ...
                Number(
                    this.runtime.getSetting("TWITTER_POLL_INTERVAL") || 300
                ) * 1000 // Default to 5 minutes
            );
        };
        handleTwitterInteractionsLoop();
    }
    ...
    async handleTwitterInteractions() {
        const now = new Date();
        elizaLogger.log("Checking Twitter interactions", now.toISOString());
