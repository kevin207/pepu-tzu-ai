...
export class MessageManager {
    ...
    private contributorChannelIds: string[] = [];
    ...
    constructor(discordClient: any, voiceManager: VoiceManager) {
        ...
        this.contributorChannelIds = [
            this.runtime.getSetting("DISCORD_CHANNEL_CONTRIBUTION_ID"),
            this.runtime.getSetting("DISCORD_CHANNEL_SETTING_GOALS_ID"),
        ];
    ...
    async handleMessage(message: DiscordMessage) {
        ...
        try {
            ...
            const roomId = stringToUuid(
                channelId + "-" + userId + "-" + this.runtime.agentId
            );
            ...
            const content: Content = {
                text: `${userName}: ${processedContent}`,
    ...
    async getUserActivities(userId: UUID) {
        const roomIds =
            await this.runtime.databaseAdapter.getRoomsForParticipant(userId);

        console.log(`Retrieved room IDs for user ${userId}:`, roomIds);

        const memories = await this.memoryManager.getMemoriesByRoomIds({
            roomIds,
        });

        elizaLogger.log("User Memories: ", memories);

        // console.log(`Retrieved memories for user ${userId}:`, memories);

        const userMemories = memories;

        // elizaLogger.log("User Memories: ", userMemories);
        // TODO: Get agent memories too; seems to have those already; should apply roles to each message
        // TODO: Rearrange the memories of agent and user in each room by time
        // TODO: Add LibriX goals into engagement analysis prompt
        return userMemories;
    }
    ...
    private async _generateResponse(
        ...
        const content: Content = {
            ...response,
            text: `${this.runtime.character.name}: ${response.text}`,
        };
        await this.runtime.databaseAdapter.log({
            body: { message, context, response: content },
            userId: userId,
            roomId,
            type: "response",
        });
        elizaLogger.log("Logged response", content);
